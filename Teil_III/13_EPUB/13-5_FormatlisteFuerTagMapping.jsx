...
function exportFormatList () {
	...
	for (var s = 0; s < _results.length; s++) {
		...
		if (_tf != null) {
			var _pagePositon = _tf.parentPage.documentOffset;
			var _pageName = _tf.parentPage.name;
		}
		else {
			var _pagePositon = "999999";
			var _pageName = "Übersatz";
		}
		_formatArray.push({name:_formatName, style:_style, pageName:_pageName, sortPositon:_pagePositon, textPosition:s});
	}
	...
	var _formatListe = "";
	var _mappingListe = "	paragraphStyles : [\r";
	
	for (var i = 0; i < _formatArray.length; i++) {
		_formatListe += _formatArray[i].name +"\tSeite: "+ _formatArray[i].pageName + "\r";
		_mappingListe +=  "{name:\"" +_formatArray[i].name + "\", exportTag:\"p\", exportClass:\"" + _formatArray[i].name + "\", emitCss:false, splitDocument:false},\r"
	}
	...
	_mappingListe += "	],"
	var _exportFile = new File("~/Desktop/out.txt");
	_exportFile  = _exportFile.openDlg("Bitte wählen Sie eine Datei zum Speichern der Formatzuordnung","*.txt");
	if (_exportFile) {
		writeTextFile(_exportFile, _mappingListe);
	}
}
...
function sortIndex (a,b) {
	if (a.pagePositon == b.pagePositon) {
		return a.textPosition - b.textPosition;
	}
	return a.pagePositon - b.pagePositon;
}
